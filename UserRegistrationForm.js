...
export default class UserRegistrationForm extends Form {
  ...
  minmaxLengthPassword(showError) {
    if (
      this.fields.password.value.length >= 3 &&
      this.fields.password.value.length <= 10
    )
      return true;
    if (showError)
      this.errorElements.password.textContent +=
        "Password must be 3- 10 symbol!";
    return false;
  }
  ...
  isEmailNotUsed(showError) {
    if (Object.keys(this.users.get(this.fields.email.value)).length === 0)
      return true;
    if (showError) this.errorElements.email.textContent += "This email used!";
    return false;
  }
